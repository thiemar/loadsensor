...
BL_LD_FLAGS = $(MCU_CC_FLAGS) $(BL_OPTIM_FLAGS) -Wl,--gc-sections $(SYS_LD_FLAGS) -Wl,-Map=$(BL_BUILD)$(BL).map -ffreestanding -nostartfiles -nostdlib -nodefaultlibs
...
image: firmware/$(PROJECT).bin
	$(PYTHON) tools/make_can_boot_descriptor.py --verbose -g firmware/$(PROJECT).bin firmware/com.thiemar.loadsensor-v1-1.0.00000000.uavcan.bin
